# Script PowerShell pour installer les JARs Kafka dans /opt/spark/jars sur tous les conteneurs Spark
...
Write-Host "📦 Installation des JARs Kafka pour Spark..." -ForegroundColor Cyan
Write-Host ""
...
Write-Host "1️⃣  Téléchargement des JARs sur spark-master..." -ForegroundColor Yellow
...
foreach ($container in $SPARK_CONTAINERS) {
    Write-Host ""
    Write-Host "2️⃣  Installation des JARs sur $container..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "Les JARs sont maintenant disponibles dans /opt/spark/jars sur tous les conteneurs Spark."
Write-Host "Vous pouvez maintenant lancer stream_kafka_to_silver.py sans --jars"
